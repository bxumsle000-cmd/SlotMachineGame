...
public class GameService {
    ...
    public int getUserBalance(){
        ...
        if(memberWallet.isEmpty()){
            throw  new IllegalArgumentException("找不到會員錢包");
        }
    ...
    public SpinResponse dospin(int betAmount){
        if (betAmount < BetConfig.MinBet){
            throw  new IllegalArgumentException("下注金額必須大於或等於50");
        }
        if (betAmount > BetConfig.MaxBet){
            throw  new IllegalArgumentException("下注金額必須小於或等於500");
        }
        if (betAmount %50 !=0){
            throw  new IllegalArgumentException("下注金額必須為50的倍數");
        }
